# 法律技能目录

本目录收录律锥·legalskill 社区已审核的法律技能，包含官方维护技能和社区精选技能。

## 收录标准

- 技能完整可用，包含 `SKILL.md` 和元数据
- 符合社区合规红线（含免责声明、标注来源）
- 采用 Apache 2.0 / CC BY-SA 4.0 或兼容许可

## 官方维护

| 技能 | 简介 | 作者 | 许可 |
|:-----|:-----|:-----|:-----|
| [tencent-ima-copilot-legal-consultation](./skill/tencent-ima-copilot-legal-consultation) | 资深律师式法律咨询，适配 ima.copilot RAG，五阶段诊断 | 律锥·legalskill | CC BY-SA 4.0 |
| [skill-writing-assistant](./skill/skill-writing-assistant) | 技能编写与修改助手，提供创建和修改标准技能的完整指南 | 律锥·legalskill | CC BY-SA 4.0 |

## 社区精选

| 技能 | 简介 | 作者 | 许可 | 来源 |
|:-----|:-----|:-----|:-----|:-----|
| （待收录） | — | — | — | — |

> 推荐技能？请在 [GitHub Discussions](https://github.com/legalskill/legalskill/discussions/categories/ideas) 发帖。

---

*最后更新：2026 年 4 月*