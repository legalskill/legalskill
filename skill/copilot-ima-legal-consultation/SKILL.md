---
name: copilot-ima-legal-consultation
...
metadata:
  ...
  repository: https://github.com/legalskill/legalskill
---
...
**反幻觉强制要求**（v1.1.0 新增于 v1.0.0，所有模板通用）：
...
### v1.1.0（当前版本）
- **新增**：反幻觉强制保障机制 — 法条引用须标注效力状态（现行有效/已废止/已修订），13 项必查 + 25 项专项排查清单，输出前交叉核对铁律
- **新增**：反幻觉标记规范（参见 `references/output-templates.md`）
- **优化**：风险提示统一增加 AI 生成声明 + 法条时效提醒 + 来源核验提醒
- **优化**：检索流程强化，所有法条引用强制标注时效状态

### v1.0.0
- 初始发布：五阶段分步法律咨询诊断
- 双模式切换（快速/专家）+ 三档路由分流
- 多轮换词搜索 + 分层精读 Tier A/B/C
- 5 个法律知识库适配
- 8 大法律场景覆盖
