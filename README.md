...
律锥·legalskill 以 **法律技能（Legal Skills）** 为核心产出，每一个技能都是可独立运行、可组合调用的智能单元。它们深度适配 IMA 知识库 RAG 能力，同时兼容通用环境，实现"一次编写，双端可用"。

### 官方维护技能

| 技能 | 简介 | 适用场景 |
|:-----|:-----|:---------|
| [legal-consultation](./skills/legal-consultation) | 资深律师式法律咨询，深度适配 IMA 知识库 RAG，五阶段分步诊断，法条与案例均可溯源验证。IMA 环境自动走 RAG 检索，非 IMA 环境自动走联网检索。 | IMA / CLI / 通用 |

### 社区精选技能

> 查看更多社区贡献的技能，请访问 [技能目录](./SKILLS_CATALOG.md)。
...
│       └── skill.yaml             # 标准化元数据
├── community_skills/              # 社区精选技能（Git 子模块）
├── .github/
├── LICENSE-CODE                   # Apache 2.0
├── LICENSE-CONTENT                # CC BY-SA 4.0
├── README.md
├── CONTRIBUTING.md
└── SKILLS_CATALOG.md              # 社区技能目录
```
...
- AI 生成内容须经人工复核后方可采用。
- 严禁将涉案涉密信息输入公域大模型。推荐使用具备本地知识库检索能力的工具（如 IMA）降低泄密风险。
...
- **贡献技能**：参考 `CONTRIBUTING.md` 和 [legal-consultation](./skills/legal-consultation) 的目录结构提交 PR。新技能须包含 `SKILL.md`（含 CC BY-SA 4.0 声明与完整免责声明）、代码文件及标准元数据 `skill.yaml`。
